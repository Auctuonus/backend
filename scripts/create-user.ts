...
async function main() {
  ...
  try {
    ...
    const tgStr = (
      await rl.question('Telegram User ID (empty = random): ')
    ).trim();
    const telegramId = tgStr
      ? parseInt(tgStr, 10)
    ...
    const pwd = (
      await rl.question("Password (empty = default '123456'): ")
    ).trim();

    let user = await UserModel.findOne({ telegramId });
    if (user) {
      console.log(
        `ℹ️  User with telegramId=${telegramId} already exists: ${String(user._id)}`,
      );
    } else {
    ...
    if (wallet) {
      console.log(
        `ℹ️  Wallet already exists: ${String(wallet._id)} (balance=${wallet.balance})`,
      );
    } else {
      ...
      console.log(
        `💰 Created wallet: ${String(wallet._id)} (balance=${wallet.balance})`,
      );
    }
    ...
    console.log(
      `  Wallet ID: ${String(wallet._id)} | balance: ${wallet.balance}, locked: ${wallet.lockedBalance}`,
    );
    console.log('\n🎉 Done');
  } catch (e) {
